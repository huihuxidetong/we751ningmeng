...
class beachprint_EweiShopV2Page extends WebPage
{
...
//			}
		...
		if( !empty($list) )
		{
			...
			{
				$value["bigPrice"] =$this-> num_to_rmb($value['price']);;
				$value["address"] =$this-> num_to_rmb($value['address']);;
				$value["realname"] =$this-> num_to_rmb($value['realname']);;
				$value["mobile"] =$this-> num_to_rmb($value['mobile']);;
				...
				foreach( $order_goods as &$og )
				{
					...
					$og["number"] = $number;
					$goods .= "序号： ".$og["number"]." 单价: " . $og["price"] / $og["total"] . " 折扣后: " . $og["realprice"] / $og["total"] . " 数量: " . $og["total"] . " 总价: " . $og["price"] . " 折扣后: " . $og["realprice"] . "\r\n ";
		...
		if( $_GPC["export"] == 1 )
		{
			...
			foreach( $list as &$row )
			{
				$row["bigPrice"] =$this-> num_to_rmb($row['price']);;
		...
		$r_type = array( "退款", "退货退款", "换货" );
		load()->func("tpl");
		include($this->template("order/beachprint"));
	}
