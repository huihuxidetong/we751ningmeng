...
class Print_EweiShopV2Page extends WebPage
{
	public function main()
	{
		...
		$bigPrice = $this-> num_to_rmb($item['price']);;
		...
		if( !empty($item["merchid"]) )
		{
			$merch_user = pdo_fetch("select * from " . tablename("ewei_shop_merch_user") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $item["merchid"], ":uniacid" => $_W["uniacid"] ));
		}
		load()->func("tpl");
