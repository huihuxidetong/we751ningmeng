...
class Goods_EweiShopV2Page extends AppMobilePage 
{
	public function get_list() 
	{
		...
		$openid = $_W["openid"];
		$level = "0";;
		$level_info = m("member")->getLevel($openid);
		...
		else 
		{
			...
			if( 0 < $goods["total"] ) 
			{
				...
				foreach( $goods_list as $index => $item ) 
				{
					...
					$goods_list[$index]['levelbuy'] = '1';
					if ($goods_list[$index]['buylevels'] != '')
					{
						$buylevels = explode(',', $goods_list[$index]['buylevels']);
						if (!(in_array($level, $buylevels)))
						{
							$goods_list[$index]['levelbuy'] = 0;
							$goods_list[$index]['canbuy'] = false;
						}
					}
					$goods_list[$index]['groupbuy'] = '1';
					if (($goods_list[$index]['buygroups'] != '') && !(empty($groupid)))
					{
						$buygroups = explode(',', $goods_list[$index]['buygroups']);
						$intersect = array_intersect($groupid, $buygroups);
						if (empty($intersect))
						{
							$goods_list[$index]['groupbuy'] = 0;
							$goods_list[$index]['canbuy'] = false;
						}
					}
					...
					$goods_list[$index]["hidegoodsprice"] = $hidegoodsprice;
					$goods_list[$index]["level"] = $level;
	...
	public function get_detail() 
	{
		...
		$goods["hidegoodsprice"] = $hidegoodsprice;
		$goods["level"] = $level;
