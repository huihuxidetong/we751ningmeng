...
class Index_EweiShopV2Page extends AppMobilePage 
{
	...
	public function getAllCart($pageid, $json = true) 
	{
		...
		$hidegoodsprice =  m('common')->getSysset('shop')['hide_goods_price'];
		$list = array( );
		$simple_list = array( );
		...
		$sql = "SELECT f.id,f.total,f.goodsid,g.total as stock,g.preselltimeend,g.buylevels,g.buygroups,g.presellprice as gpprice,g.hasoption, o.stock as optionstock,g.presellprice,g.ispresell, g.maxbuy,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice," . " g.productprice,o.title as optiontitle,o.presellprice,f.optionid,o.specs,g.minbuy,g.maxbuy,g.unit,f.merchid,g.checked,g.isdiscount,g.isdiscount_discounts,g.isdiscount_time,g.isnodiscount,g.discounts,g.merchsale" . " ,f.selected FROM " . tablename($tablename) . " f " . " left join " . tablename("ewei_shop_goods") . " g on f.goodsid = g.id " . " left join " . tablename("ewei_shop_goods_option") . " o on f.optionid = o.id " . " where 1 " . $condition . " ORDER BY `id` DESC ";
		$list = pdo_fetchall($sql, $params);
		...
		foreach( $list as &$g ) 
		{
			...
			$g["hidegoodsprice"] = $hidegoodsprice;
			$g["level"] = $level['level'];
			$g['levelbuy'] = '1';
			if ($g['buylevels'] != '')
			{
				$buylevels = explode(',', $g['buylevels']);
				if (!(in_array($level, $buylevels)))
				{
					$g['levelbuy'] = 0;
					$g['canbuy'] = false;
				}
