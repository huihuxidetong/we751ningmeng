...
class Create_EweiShopV2Page extends AppMobilePage
{
	...
	public function main()
	{
		...
		if( !$packageid )
		{
			...
			if( empty($id) )
			{
				$array = array( ":uniacid" => $uniacid, ":openid" => $openid);
				if( !empty($quickid) )
				{
					...
					if((!empty($formmerchid))){
						$sql .= " and c.merchid=:merchid ";
						$array = array( ":uniacid" => $uniacid, ":openid" => $openid,":merchid"=> $formmerchid );
					}
	...
	public function submit()
	{
		...
		if( $_GPC["fromcart"] == 1 )
		{
			$formmerchidcondition = "";
			if(!empty($formmerchid)){
				$formmerchidcondition = " and merchid=:merchid";
			}
			$sql = "update " . tablename("ewei_shop_member_cart") . " set deleted=1 where  openid=:openid and uniacid=:uniacid and selected=1 " . $formmerchidcondition;
			pdo_query($sql, array( ":uniacid" => $uniacid, ":openid" => $openid, ":merchid" => $formmerchid ));
		}
		if( p("quick") && !empty($_GPC["fromquick"]) )
		{
