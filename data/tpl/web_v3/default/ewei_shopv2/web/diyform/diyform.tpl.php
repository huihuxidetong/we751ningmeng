<?php defined('IN_IA') or exit('Access Denied');?>
    ...
    <?php  if(!empty($datas[$key]) && !empty($datas[$key]['0']) && !empty($datas[$key]['1'])) { ?>
    <?php  if(is_array($datas[$key])) { foreach($datas[$key] as $k1 => $v1) { ?>
    <?php  echo $v1?>
    <?php  } } ?>
    <?php  } else { ?>
    ...
    <?php echo $datas[$key]['province']!='请选择省份'&&$datas[$key]['province']!=''?$datas[$key]['province']:'未填写'?>-<?php echo $datas[$key]['city']!='请选择城市'&&$datas[$key]['city']!=''?$datas[$key]['city']:'未填写'?><?php  if(!empty($datas[$key]['area'])) { ?>-<?php  echo $datas[$key]['area']?><?php  } ?>
