...
<div class="page-header">当前位置：<span class="text-primary">商城设置</span></div>
    ...
    <div class="page-content">
        <form action="" method="post" class="form-horizontal form-validate" enctype="multipart/form-data" >
            ...
            <div class="form-group">
                <label class="col-lg control-label">公开显示产品价格</label>
                <div class="col-sm-9 col-xs-12">
                    <?php if(cv('sysset.shop.edit')) { ?>
                    <label class="radio-inline"><input type="radio" name="data[hide_goods_price]" value="0" <?php  if(empty($data['hide_goods_price'])) { ?>checked=""<?php  } ?>> 开启</label>
                    <label class="radio-inline"><input type="radio" name="data[hide_goods_price]" value="1" <?php  if(!empty($data['hide_goods_price'])) { ?>checked=""<?php  } ?>> 关闭</label>
                    <?php  } else { ?>
                    <div class='form-control-static'><?php  if(empty($data['hide_goods_price'])) { ?>开启<?php  } else { ?>关闭<?php  } ?></div>
                    <?php  } ?>
                    <div class="help-block"> 如果开启此选项，非会员商品价格将会显示</div>
                </div>
            </div>
            <div class="form-group">
                <label class="col-lg control-label">出款方账户名</label>
                <div class="col-sm-9 col-xs-12">
                    <?php if(cv('sysset.shop.edit')) { ?>
                    <input type="text" id="accountname" name="data[accountname]" class="form-control" value="<?php  echo $data['accountname'];?>" />
                    <?php  } else { ?>
                    <input type="hidden" name="data[accountname]" value="<?php  echo $data['accountname'];?>"/>
                    <div class='form-control-static'><?php  echo $data['accountname'];?></div>
                    <?php  } ?>
                </div>
            </div>
            <div class="form-group">
                <label class="col-lg control-label">出款方账户</label>
                <div class="col-sm-9 col-xs-12">
                    <?php if(cv('sysset.shop.edit')) { ?>
                    <input type="text" id="account" name="data[account]" class="form-control" value="<?php  echo $data['account'];?>" />
                    <?php  } else { ?>
...
<script type="text/javascript">
    $(function () {
        $('form').submit(function(){
            ...
            if(account_name == ''){
                $('form').attr('stop',1),tip.msgbox.err('请填写出款方账户名!');
                flag = false;
            }
            if(account == ''){
                $('form').attr('stop',1),tip.msgbox.err('请填写出款方账户!');
                flag = false;
            }
            if(flag) {
